...
pub(super) async fn list_integration_sync_feed(
    ...
    let items = crate::core::integration_sync::list_feed_items(
        &ctx,
...
pub(super) async fn list_integration_sync_runs(
    ...
    let page = crate::core::integration_sync::list_runs(
        &ctx,
        query.integration_id.as_deref(),
        query.limit.unwrap_or(20),
...
pub(super) async fn update_integration_sync_config(
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(request): Json<crate::core::integration_sync::IntegrationSyncUpdateRequest>,
) -> Response {
    let shared_agent = state.agent.clone();
    let ctx = {
        let agent = shared_agent.read().await;
    ...
    match crate::core::integration_sync::update_config(&ctx, &id, request).await {
        Ok(status) => (
            StatusCode::OK,
...
pub(super) async fn run_integration_sync_now(
    ...
    match crate::core::integration_sync::sync_now(&ctx, &id).await {
        Ok(status) => (
            StatusCode::OK,
            Json(serde_json::json!({ "status": "ok", "sync": status })),
        )
