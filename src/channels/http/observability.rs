...
pub(super) fn build_observability_settings_response(
    config: &crate::core::config::ObservabilityConfig,
    config_dir: &FsPath,
    data_dir: &FsPath,
) -> ObservabilitySettingsResponse {
    let auth_token_configured =
        crate::core::observability::has_observability_auth_token(config_dir, Some(data_dir))
            .unwrap_or(false);
    ObservabilitySettingsResponse {
        enabled: config.enabled,
        provider: crate::core::observability::normalize_observability_provider(&config.provider),
        ...
        header_name: crate::core::observability::normalize_observability_header_name(
            &config.header_name,
        ),
        privacy_mode: crate::core::observability::normalize_observability_privacy_mode(
            &config.privacy_mode,
        ),
...
pub(super) async fn get_observability_logs(
    ...
    let issues = crate::core::observability::summarize_log_issues(&logs);
...
pub(super) async fn test_observability_export(State(state): State<AppState>) -> Response {
    ...
    match crate::core::observability::export_test_trace(&config, &config_dir, &data_dir, &storage)
        .await
    {
