 ...
 *   BRIDGE_HOST       – Host this bridge listens on (default: 127.0.0.1)
 *   BRIDGE_PORT       – Port this bridge listens on (default: 8999)
 *   BRIDGE_TOKEN      – Optional shared token protecting bridge routes
...
function requireBridgeToken(req, res, next) {
  ...
  const token = (req.get("x-agentark-bridge-token") || "").trim();
...
app.get("/qr.png", requireBridgeToken, (_req, res) => {
  if (!currentQR) {
...
app.post("/presence", requireBridgeToken, async (req, res) => {
...
app.post("/send-video", requireBridgeToken, async (req, res) => {
...
app.post("/send-image", requireBridgeToken, async (req, res) => {
